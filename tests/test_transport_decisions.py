...
成本模型本身（成本原语 / 货拉拉计价 / TCO 分析 / 三档敏感性 / 自营 vs 外包对照）已收口到
`src/costing.py`，其回归测试在 `tests/test_costing.py`——本模块只消费它，不再自带一份。
"""
...
class TestModeCostBeforeAfter:
    ...
    def test_daily_cost_is_fixed_plus_mileage_per_trip(self):
        ...
        d = got["diesel"]
        fixed, per_km = costing.fixed_per_day("diesel"), costing.per_km("diesel")
