...
一次运行产出成都冷链城配的完整配送情景：
  ...
  ② **车辆表与 TCO 产物**（`vehicles.csv` + `tco_analysis.{json,md}`）：15 台 4.2 米
     冷链轻卡（柴油自购 / 纯电租赁双模式），落盘双模式日总成本曲线、盈亏平衡里程、
     司机工资/能源价格/租金三档敏感性、自营单趟成本 vs 货拉拉对照表与模式建议。
     **成本模型本身住在 `src/costing.py`**——本模块只调用它并落盘；收口前计价规则与
     TCO 分析长在这里，于是「数据生成器持有业务定价规则」成了本仓库最别扭的一条依赖边。
        ...
        rows.append(
            {
                ...
                "fixed_cost_per_day": costing.fixed_per_day(mode),
                "cost_per_km": costing.per_km(mode),
