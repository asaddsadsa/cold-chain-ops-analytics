...
#:
#: 取值是从锚点校准出来的，不是挑的：校准基准是 `WAREHOUSE_RELEASE_DELAY_MIN` 的**参数
#: 均值** 25 分钟。层 A 的**实测**「下单 → 开始拣货」平均等待是 1396 秒（单口径，23.3
#: 分钟）／1615 秒（行口径，26.9 分钟）——与参数均值同量级，差在行内错峰与逐日抽样。
#: 波次窗内订单近似均匀到达，平均累积等待 = W/2，于是 W = 2 × 25 = 50 分钟。
#:
...
# ---------------------------------------------------------------------------
# 车辆与成本参数锚点（全部情景假设；具体来源 URL 已于 2026-09-15 回填核验，见台账第 4 节）
...
#: 其余为校准值 `WAREHOUSE_WAVE_INTERVAL_MIN` 的 1/2、1、2、4 倍——由它派生而不是写死，
#: 否则改了校准值以后这几档就不再是「倍数」，而注释还写着是。
SIM_WAVE_LEVELS: tuple[float, ...] = (
    0.0, 0.5 * WAREHOUSE_WAVE_INTERVAL_MIN, WAREHOUSE_WAVE_INTERVAL_MIN,
    2.0 * WAREHOUSE_WAVE_INTERVAL_MIN, 4.0 * WAREHOUSE_WAVE_INTERVAL_MIN,
)
...
#:
#: 为什么是 500 而不是更小的数：实测三个 Solomon 算例在 500 处的距离与「30 秒墙钟」时代
#: **逐位相同**（C101 829.01 / R101 1653.54 / RC101 1669.40），而 200 处 R101 与 RC101
#: 会偏大（1680.68 / 1682.15）——因为 30 秒对这两个算例跑得比 200 个解更久。
#: 取 500 同时拿到「可复现」与「质量不降」，耗时 15–40 秒/算例，与原来的 30 秒档相当。
VRPTW_SOLUTION_LIMIT: int = 500
