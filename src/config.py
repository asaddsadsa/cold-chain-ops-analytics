...
# SimPy 代表日规模与到达集中度（情景假设，登记台账）
# 注意：300 单/天 × ~100s/单 ≈ 3 万秒工作量，对 4–6 拣货员（10h 窗口=14.4 万秒/人）
# 属欠载系统，利用率 0.15–0.3、人力档位差异小——这是需求「日均 200–400 单」与
# 「4/5/6 人拐点」之间的内在张力，诚实结论见 calibration 与 07 号票实现记录。
SIM_N_ORDERS_PER_DAY: int = 300  # 代表日订单数（数据层 A 日均 200–400 中值）
SIM_PEAK_INTENSITY: float = 10.0  # NHPP 高峰/平峰强度比（波次拣货到达集中度）
