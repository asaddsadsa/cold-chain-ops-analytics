...
#:
#: **代价已如实记录**：统一后实验二「拐点在 5 人」的结论不再成立——4→5 人的边际收益降到
#: 0.0005 秒/元（等于没有），见 `data/processed/sim/exp2_staffing.json` 与台账。
...
# 属欠载系统，利用率 0.14–0.21、人力档位差异小——这是需求「日均 200–400 单」与
# 「4/5/6 人拐点」之间的内在张力。**这个张力是真的**：把到达强度调高（旧值 10.0）
# 确实能造出一个拐点，但那是人造的（见 `WAREHOUSE_PEAK_INTENSITY` 的说明）。
# 诚实结论：在一致的到达强度下，4 人档已经够用，4–6 人之间分不出拐点。
...
# 优化与门禁（模块二 / 数据层 E）
# ---------------------------------------------------------------------------
VRPTW_TIME_LIMIT_SEC: float = 30.0  # 代表日 OR-Tools 求解时限
...
#: what-if 每档 VRPTW 求解时限（秒）。ADR-0010 定车辆数 5–20 共 16 档离线预计算，
#: 每档按代表日规模跑一次，合计约 8 分钟——一次性成本换看板滑块零等待。
WHATIF_TIME_LIMIT_SEC: float = 30.0
